...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var GLASS_CSS = [
      ...
      'body { background-color: color-mix(in srgb, #0d1117 calc(var(--dsh-glass-alpha) * 100%), var(--dsw-alias-bg-base)) !important; }',
      // Wallpaper active: the z-index:-1 wallpaper layer paints above the html
      // background but below in-flow content, so body's opaque color-mix tint
      // would hide it. Switch body to transparent and the layer's own dark
      // overlay keeps chat text readable.
    ...
    function ensureWallpaperLayer() {
      ...
      if (wallpaperUrl) {
        el.style.backgroundImage = 'linear-gradient(rgba(13,17,23,0.55), rgba(13,17,23,0.55)), url("' + wallpaperUrl + '")'
    ...
    function CursorFxControl() {
      ...
      var modeOpts = function (value, on) {
        ...
        for (var oi = 0; oi < opts.length; oi++) {
          var o = opts[oi]
          out.push(h('option', { value: o[0], selected: value === o[0] ? true : undefined }, o[1]))
        }
        return out
      }
