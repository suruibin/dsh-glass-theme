...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var KEYS = {
      ...
      enabled: 'dsh-glass.enabled',
      brandInterval: 'dsh-glass.brandInterval',
      desktopEnabled: 'dsh-glass.desktopEnabled',
    }
    var DEFAULT_ALPHA = 0.4
    var DEFAULT_FX = { enabled: true, sidebar: 'star', center: 'water' }
    var DEFAULT_BRAND_CYCLE = true
    var DEFAULT_ENABLED = true
    var DEFAULT_BRAND_INTERVAL = 10 // 秒
    var DEFAULT_DESKTOP_ENABLED = false
    ...
    var themeColorHandle = null // theme-color meta watcher, disposed with the effect
    ...
    var GLASS_CSS = [
      ...
      // software rendering (see hero below). Master-switch gated: off leaves
      // the page fully native for other skins.
      'html[data-dsh-glass-enabled="1"]::after {',
      "  content: '';",
      ...
      '}',
      'html[data-dsh-glass-enabled="1"]::before {',
      "  content: '';",
      '  position: fixed; inset: 0;',
      '  z-index: 2147483000;',
      '  pointer-events: none;',
      // Neutral top shade instead of the old brand-blue glow: a full-screen
      // blue tint painted over wallpaper / skins and read as "why is my
      // background blue?" Keep the top-depth feel but drop the hue so the
      // wallpaper's real colors show through untouched.
      '  background: radial-gradient(120% 70% at 50% -8%, rgba(20, 24, 32, 0.18), transparent 62%);',
      '}',
      ...
      // (wallpaper/tint shows through), 1 → fully opaque. Gated by the
      // glass-effects master switch: when off, DSH's own opaque token values
      // apply untouched, so other skins (e.g. the skin center) render cleanly.
      'body[data-dsh-glass-enabled="1"][data-ds-dark-theme] {',
      ...
      '}',
      'body[data-dsh-glass-enabled="1"]:not([data-ds-dark-theme]) {',
      ...
      '}',
      // No opaque tint on html: it (and body) stay transparent so whatever
      // sits behind the page — the wallpaper layer, the desktop, or the
      // host's own background — shows through the translucent panel tokens
      // above. An opaque html color would read as a solid dark block
      // whenever no wallpaper is set or after it is removed. Master switch
      // gates both: off restores the host's own backgrounds.
      'html[data-dsh-glass-enabled="1"] { background-color: transparent !important; }',
      'body[data-dsh-glass-enabled="1"] { background-color: transparent !important; }',
      ...
      'body[data-dsh-glass-enabled="1"][data-dsh-glass-wallpaper="1"] { background-color: transparent !important; }',
    ].join('\n')
    ...
    // ── 运行环境识别 ─────────────────────────────────────
    // 桌面版(Deepseek-Harness Electron 壳)通过 preload 注入 window.dshDesktop
    // 桥;浏览器版没有。桌面版也跑 dsh web(默认 web profile),插件会跟着
    // 生效,与桌面版自带的 glass.ts 效果打架——所以桌面版专属开关默认关闭,
    // 桌面版里插件完全不注入样式,只留设置页。
    function isDesktopShell() {
      return typeof window !== 'undefined' && typeof window.dshDesktop !== 'undefined'
    }
    function loadDesktopEnabled() {
      try {
        var raw = localStorage.getItem(KEYS.desktopEnabled)
        if (raw === 'true' || raw === 'false') return raw === 'true'
      } catch (e) {}
      return DEFAULT_DESKTOP_ENABLED
    }
    // ── 玻璃效果总开关:关闭后透明面板 token 与 html/body 透明全部失效
    //    (CSS 由 data-dsh-glass-enabled 门控),壁纸层不挂载——装皮肤中心
    //    等其他皮肤时切回 DSH 原生外观,互不干扰。──
    function loadEnabled() {
      try {
        var raw = localStorage.getItem(KEYS.enabled)
        if (raw === 'true' || raw === 'false') return raw === 'true'
      } catch (e) {}
      return DEFAULT_ENABLED
    }
    function applyEnabled(on) {
      document.documentElement.setAttribute('data-dsh-glass-enabled', on ? '1' : '0')
      document.body.setAttribute('data-dsh-glass-enabled', on ? '1' : '0')
      try { localStorage.setItem(KEYS.enabled, on ? 'true' : 'false') } catch (e) {}
      if (wallpaperUrl) { try { ensureWallpaperLayer() } catch (e) {} }
      // theme-color 同步启停:关掉玻璃主题时把 meta 还给 DSH/皮肤,让其他
      // 皮肤自行决定浏览器标题栏颜色;开启时接管并钉住玻璃色。
      if (on && themeColorHandle === null) {
        writeThemeColor()
        themeColorHandle = startThemeColorWatch()
      } else if (!on && themeColorHandle !== null) {
        try { themeColorHandle.dispose() } catch (e2) {}
        themeColorHandle = null
    ...
    function ensureWallpaperLayer() {
      ...
      // 总开关关闭时不挂壁纸层,并摘掉 body 上的 wallpaper 属性(透明 CSS
      // 已被 enabled 门控,此属性留着会让 body[data-dsh-glass-enabled="1"]
      // 规则失效时残留;顺带清理)。
      if (!loadEnabled()) {
      ...
      if (wallpaperUrl) {
        // Mask follows the alpha slider (0.30-0.70, never fully opaque) so
        // text stays readable over the wallpaper (html/body are transparent).
        // Pure black tint — no blue cast — so the wallpaper keeps its real
        // colors, only darkened.
        var a = loadAlpha() // 0.4-1
        var mask = Math.min(1, Math.max(0.3, a * 0.7)).toFixed(2)
        el.style.backgroundImage = 'linear-gradient(rgba(0,0,0,' + mask + '), rgba(0,0,0,' + mask + ')), url("' + wallpaperUrl + '")'
    ...
    function loadBrandInterval() {
      try {
        var n = parseInt(localStorage.getItem(KEYS.brandInterval), 10)
        if (!isNaN(n) && n >= 3 && n <= 300) return n
      } catch (e) {}
      return DEFAULT_BRAND_INTERVAL
    }
    function startBrandCycle() {
      ...
      var cycleBrandColors = function () {
        var now = Date.now()
        // 间隔可配置:每次心跳重读,设置页改动后 ≤1s 内自动生效。
        if (now - lastBrandColorChange < loadBrandInterval() * 1000) return
    ...
    function GlassEnabledControl() {
      // 总开关:关闭时透明面板 / 壁纸全部失效,方便切皮肤中心等其他皮肤。
      // 打开/关闭即时生效(CSS 门控 + 壁纸层重建)。
      var on = loadEnabled()
      var set = function (e) {
        on = e.target.checked
        applyEnabled(on)
      }
      return h('div', { 'data-dsh-glass-enabled-control': '', style: { padding: '14px 0', borderBottom: '1px solid rgba(128,132,142,0.16)' } },
        h('div', { style: { color: 'var(--dsw-alias-label-primary)', fontSize: '14px', lineHeight: '22px' } }, '玻璃主题效果'),
        h('label', { style: { display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--dsw-alias-label-secondary)', fontSize: '13px', cursor: 'pointer', marginTop: '4px' } },
          h('input', { type: 'checkbox', defaultChecked: on, onChange: set, style: { width: '15px', height: '15px', accentColor: '#4176e6', cursor: 'pointer' } }),
          '启用透明面板与背景壁纸(关闭后恢复原生外观,便于使用其他皮肤)'),
      )
    }

    function BrandCycleControl() {
      var setIntervalSec = function (e) {
        var v = parseInt(e.target.value, 10)
        if (isNaN(v) || v < 3 || v > 300) return
        try { localStorage.setItem(KEYS.brandInterval, String(v)) } catch (err) {}
        // 心跳每秒重读间隔,无需重启循环
      }
      ...
      return h('div', { 'data-dsh-brand-cycle': '', style: { padding: '14px 0' } },
        ...
        h('label', { style: { display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--dsw-alias-label-secondary)', fontSize: '13px', cursor: 'pointer', marginTop: '4px' } },
          ...
          '自动切换品牌色(logo / 侧边栏图标 / 折叠栏)'),
        h('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', marginTop: '8px' } },
          h('span', { style: { color: 'var(--dsw-alias-label-secondary)', fontSize: '13px', minWidth: '52px' } }, '间隔(秒)'),
          h('input', {
            type: 'number', min: '3', max: '300', step: '1',
            defaultValue: String(loadBrandInterval()),
            onChange: setIntervalSec,
            style: { flex: '1', background: 'rgb(39,46,62)', color: 'var(--dsw-alias-label-primary)', border: 'none', borderRadius: '18px', padding: '6px 12px', fontSize: '13px', outline: 'none' },
          }),
        ),
      )
    }

    function DesktopShellControl() {
      // 仅桌面版(Deepseek-Harness Electron)显示:插件是否在桌面版里注入
      // 玻璃效果。默认关闭——桌面版自带 glass.ts,插件不干预;打开后与
      // 浏览器版一致。
      var set = function (e) {
        var on = e.target.checked
        try { localStorage.setItem(KEYS.desktopEnabled, on ? 'true' : 'false') } catch (err) {}
        // 即时生效:切换后重建效果(关闭→移除,开启→注入)。
        location.reload()
      }
      return h('div', { 'data-dsh-desktop-control': '', style: { padding: '14px 0', borderBottom: '1px solid rgba(128,132,142,0.16)' } },
        h('div', { style: { color: 'var(--dsw-alias-label-primary)', fontSize: '14px', lineHeight: '22px' } }, '桌面版(Electron)'),
        h('label', { style: { display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--dsw-alias-label-secondary)', fontSize: '13px', cursor: 'pointer', marginTop: '4px' } },
          h('input', { type: 'checkbox', defaultChecked: loadDesktopEnabled(), onChange: set, style: { width: '15px', height: '15px', accentColor: '#4176e6', cursor: 'pointer' } }),
          '在桌面版中启用玻璃主题(默认关闭,避免与桌面版自带效果冲突)'),
      )
    }
    ...
    function SettingsPage(props) {
      return h('div', { 'data-dsh-glass-page': '' },
        ...
        isDesktopShell() ? h(DesktopShellControl) : null,
        h(GlassEnabledControl),
        h(WallpaperControl),
        h(AlphaSlider),
        h(CursorFxControl),
        h(BrandCycleControl),
    ...
    // ── 浏览器标题栏主题色 ────────────────────────────────
    // DSH 的 ThemePresenter 会把 theme-color meta 同步为 body 的计算背景色;
    // 插件把 body 变成透明后,该值成为 rgba(0,0,0,0),Chrome 应用窗口/PWA 的
    // 标题栏在深色系统下按黑色渲染,与玻璃主题脱节。这里接管该 meta,固定为
    // 与主题匹配的玻璃色,并用观察器在 DSH 每次重写后写回。
    var THEME_COLOR_DARK = '#0f1117'
    var THEME_COLOR_LIGHT = '#f5f6f7'
    var themeColorMeta = null
    var themeColorWatcher = null
    function themeColorForBody() {
      return document.body.hasAttribute('data-ds-dark-theme') ? THEME_COLOR_DARK : THEME_COLOR_LIGHT
    }
    function writeThemeColor() {
      var meta = document.querySelector('meta[name="theme-color"]')
      if (meta === null) {
      ...
      themeColorMeta = meta
      meta.setAttribute('content', themeColorForBody())
    }
    function startThemeColorWatch() {
      if (themeColorWatcher !== null) { themeColorWatcher.disconnect(); themeColorWatcher = null }
      var obs = new MutationObserver(function () {
        // DSH re-syncs the meta on every theme snapshot (typically to the
        // now-transparent body color) — write our glass color back. Writing an
        // unchanged value re-triggers this callback but the guard below makes
        // it a no-op, so there is no loop.
        var meta = document.querySelector('meta[name="theme-color"]')
        if (meta === null) {
          writeThemeColor()
        } else {
          themeColorMeta = meta
          if (meta.getAttribute('content') !== themeColorForBody()) {
            meta.setAttribute('content', themeColorForBody())
          }
        }
      })
      themeColorWatcher = obs
      obs.observe(document.head, { childList: true, subtree: true, attributes: true, attributeFilter: ['content'] })
      return {
        dispose: function () {
          obs.disconnect()
          themeColorWatcher = null
        },
      }
    }
    ...
    // ── 挂载 ──────────────────────────────────────────────
    function apply(ctx) {
      // 桌面版隔离:Electron 壳默认不加载玻璃效果(自带 glass.ts),只注册
      // 设置页以便用户在桌面版里打开开关;打开后照常注入。
      if (isDesktopShell() && !loadDesktopEnabled()) {
        console.log('[dsh-glass] desktop shell without glass enabled — settings page only')
        registerSettings(ctx)
        return
      }
      try { ensureStyle() } catch (e) { console.warn('[dsh-glass] style inject failed', e) }
      try { applyEnabled(loadEnabled()) } catch (e) { console.warn('[dsh-glass] enabled init failed', e) }
      ...
      // theme-color 接管已并入 applyEnabled(总开关开时才启用)。
      ctx.effect(function () {
        return function () {
          var tag = document.getElementById(STYLE_ID)
          if (tag) tag.remove()
          ...
          if (themeColorHandle && themeColorHandle.dispose) { try { themeColorHandle.dispose() } catch (e) {} }
